...
def get_player_cards(room_id: str):
    try:
        ...
        if existing:
            return {
                "status": 200,
                "result": [player.to_dict() for player in existing]
            }
        else:
            return {"status": 400, "message": "Не найдено карточек игроков в этой комнате"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Ошибка при выборке карточек игроков: {e}")


@router.get("/create_ai_player_card")
def create_ai_player_cards(room_id: str, user_id: str):
    ...
    try:
        ...
        return {"status": 400, "message": f"Карточка игрока с user_id={user_id} уже существует."}
    except Exception as e:
        raise HTTPException(
            status_code=500,
